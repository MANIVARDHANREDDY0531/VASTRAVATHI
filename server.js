...
function handleValidateCoupon(request, response) {
  try {
    ...
    sendJson(response, 200, {
      ...
      taxableAmount: pricing.taxablePaise,
      gstAmount: pricing.gstPaise,
...
function calculatePlanPricing(planId, couponCode) {
  ...
  if (!code || !originalPaise) {
    const gstPaise = Math.round(originalPaise * GST_RATE);
    return {
      originalPaise,
      discountPaise: 0,
      taxablePaise: originalPaise,
      gstPaise,
      payablePaise: originalPaise + gstPaise,
      coupon: null
    };
  }
  ...
  const taxablePaise = Math.max(100, originalPaise - discountPaise);
  const gstPaise = Math.round(taxablePaise * GST_RATE);
  const payablePaise = taxablePaise + gstPaise;
...
function buildPaymentUrl(planId, plan, referenceId) {
  const pricing = calculatePlanPricing(planId, '');
  if (plan.checkoutUrl) {
    const url = new URL(plan.checkoutUrl);
    url.searchParams.set('reference', referenceId);
    url.searchParams.set('plan', planId);
    url.searchParams.set('amount', String(pricing.payablePaise / 100));
    return url.toString();
  }
  const params = new URLSearchParams({
    reference: referenceId,
    plan: planId,
    amount: String(pricing.payablePaise / 100)
  });
...
async function handlePurchaseRequest(request, response) {
  try {
    ...
    const record = {
      ...
      taxableAmount: pricing.taxablePaise / 100,
      gstAmount: pricing.gstPaise / 100,
